...
const CONFIG = {
  bgColor: '#05050A',
  flameColor: '#06b6d4',      // Cyan flame
  flameColor2: '#8b5cf6',     // Violet flame
  flameAmt: 0.2,
  atmoColor: '#22d3ee',       // Light cyan motes
  atmoCount: 300,
  atmoSize: 24,
  atmoSpeed: 1.0,
  coreColor: '#000000',       // Dark core
  midColor: '#8b5cf6',        // Violet mid
  rimColor: '#06b6d4',        // Cyan rim
  opacity: 2,
  pointSize: 80,
  brightness: 1.6,
  spin: 0.03,
...
export default function AmbientBackground() {
  ...
  useEffect(() => {
    ...
    // Create canvas dynamically to avoid React Strict Mode context conflicts
    ...
    // Use WebGLRenderer and cap pixel ratio for performance
    const renderer = new THREE.WebGLRenderer({ canvas, antialias: true, alpha: false });
    const pixelRatio = Math.min(window.devicePixelRatio, 1.25); // Cap to 1.25x to prevent lag/stutter on high-DPI screens
    renderer.setPixelRatio(pixelRatio);
    renderer.setSize(width, height);

    const scene = new THREE.Scene();
    scene.background = new THREE.Color(CONFIG.bgColor);
    scene.fog = new THREE.Fog(CONFIG.bgColor, 0, 15);
    ...
    const stormObj = new THREE.Points(geometry, material);
    stormObj.layers.enable(LAYERS.ENTIRE_SCENE);
    const stormGroup = new THREE.Group();
    stormGroup.add(stormObj);
    scene.add(stormGroup);

    const N = CONFIG.atmoCount;
    const atmoPositions = new Float32Array(N * 3), sizes = new Float32Array(N), seeds = new Float32Array(N);
    for (let i = 0; i < N; i++) {
      atmoPositions[i*3] = 2*Math.random()-1; atmoPositions[i*3+1] = 2*Math.random()-1; atmoPositions[i*3+2] = 2*Math.random()-1;
    ...
    const handleMouseOut = () => { POINTER.active = false; };
    ...
    const handleResize = () => {
      ...
      atmoMat.uniforms.uRes.value.set(width * pixelRatio, height * pixelRatio);
      updateScroll();
    };
  ...
  return (
    <div 
      ref={containerRef} 
      className="fixed inset-0 z-[-1] pointer-events-none w-full h-full bg-[#05050A]"
    />
  );
}
